...
public class EventEmitterHandler implements RequestHandler<SQSEvent, Void> {
    ...
    public Void handleRequest(SQSEvent event, Context lambdaContext) {
        ...
        for (SQSEvent.SQSMessage message : event.getRecords()) {
            try {
                ...
                service.emit(listingId);
            } catch (Exception e) {
                throw new RuntimeException("Event emission failed: " + e.getMessage(), e);
            }
        }
        return null;
    }
